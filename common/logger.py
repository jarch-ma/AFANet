...
class AverageMeter:
    ...
    def update(self, inter_b, union_b, class_id, loss):
        self.intersection_buf.index_add_(1, class_id, inter_b.float())  # index_add_ 在指定维度上直接加法，原地修改输入张量
    ...
    def compute_iou(self):
        iou = self.intersection_buf.float() / \
              torch.max(torch.stack([self.union_buf, self.ones]), dim=0)[0]   #计算最大面积区域，避免0的情况
        iou = iou.index_select(1, self.class_ids_interest)
        miou = iou[1].mean() * 100   # iou[1] 感兴趣类别IOU   *100 转化为百分比的形式
