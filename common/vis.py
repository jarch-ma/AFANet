...
class Visualizer:
    ...
    def initialize(cls, visualize,vis_path):
        ...
        cls.colors = {'red': (195, 33, 54), 'blue': (75, 92, 196), 'green': (0,224,121)}
    ...
    def visualize_prediction_batch(
            cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx, iou_b=None,query_name=None):
        ...
        pred_mask_b = utils.to_cpu(pred_mask_b) # pred mask
        cls_id_b = utils.to_cpu(cls_id_b)
        cls_id_b += 1
        # query_name = utils.to_cpu(query_name)
    ...
    def visualize_prediction(
            cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label, iou=None,query_name=None):

        spt_color = cls.colors['blue']
        qry_color = cls.colors['red']  # 原来是 ['red']
        ...
        # 合并后的图片分别是： support mask；pred mask； GT mask
        ...
        merged_pil.save(cls.vis_path + 'query_%s-class_%d-iou_%.2f' % (query_name, cls_id, iou) + '.jpg')
        
        #merged_pil.save(cls.vis_path + 'batch-%d_sample-%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '.jpg')
        # batch0_3_class-5_iou-0.91.jpg
    @classmethod
