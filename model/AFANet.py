...
class afanet(nn.Module):
    ...
    def __init__(self, backbone, use_original_imgsize, benchmark, clip_model):
        ...
        if backbone == 'vgg16':
            ...
            ckpt = torch.load('/opt/data/private/Code/new_work/BBQ_1/Pretrain/vgg16-397923af.pth', weights_only=True)
            ...
            ckpt = torch.load('/opt/data/private/Code/new_work/BBQ_1/Pretrain/resnet50-19c8e357.pth', weights_only=True)
            self.backbone.load_state_dict(ckpt)    
    ...
    def forward(self, query_img, support_img, support_cam, query_cam,
        ...
        with torch.no_grad():
            ...
            if len(query_feats) == 7: # VGG Backbone
                ...
                # frequency domain              
                support_fam_low = self.vgg_fam(support_low)   # ([bsz, 64, 50, 50])
                support_fam_mid = self.vgg_fam(support_mid)   # ([bsz, 64, 25, 25])
                support_fam_hig = self.vgg_fam(support_hig)   # ([bsz, 64, 12, 12])
                ...
                clip_text_features = self.linear_1024_625(
                    self.clip_text_features.float().to(support_ncd_feats.device))  # torch.Size([20, 625])
